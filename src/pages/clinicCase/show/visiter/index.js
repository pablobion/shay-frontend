                ...
                {currentChecklist ? (
                    <>
                        <Box mb={2} bg={useColorModeValue("gray.100", "gray.700")} borderRadius="lg" p={5} w="100%">
                            ...
                            <Textarea readOnly style={textareaStyle}>{currentChecklist.roadmap}</Textarea>
                        </Box>
                        <Box mb={2} bg={useColorModeValue("gray.100", "gray.700")} borderRadius="lg" p={5} w="100%">
                            <Heading color={useColorModeValue("teal.700", "white")} fontSize="2xl">
                                <Text>Roteiro</Text>
                                            ...
                                            <Flex justify='space-between'>
                                                <Box w="100%">
                                                    <Flex borderRadius={10}>
                                                        <Box w="90%">
                                                            <Textarea readOnly sx={{border: 'none'}} style={textareaStyle}>{question.text}</Textarea>
                                                        </Box>
                                                        <Box ml={2}></Box>
                                                    </Flex>
                                                    <Text>{question.comment}</Text>
                                                </Box>
                                                <Flex gap={3} mr={10} >{question?.points && (question?.points).map((point, index) => <Button>{point}</Button>)}</Flex>
                                            </Flex>
                                        </>
                                    ))}
                            </Box>
