import { Select, Box, Button, Flex, Icon, Input, InputGroup, InputLeftElement, InputRightElement, Table, TableContainer, Tbody, Td, Text, Th, Badge, Thead, Tr } from "@chakra-ui/react";
                                ...
                                {headerGroup.headers.map((column) => (
                                    <Th {...column.getHeaderProps(column.getSortByToggleProps())}>
                                        <Flex align={"center"} gap={"10px"}>
                                            ...
                                            {column.isSorted && <Box as="span">{column.isSortedDesc ? <ArrowDownIcon boxSize={3} ml={2} /> : <ArrowUpIcon boxSize={3} ml={2} />}</Box>}
                                            <Box ml={2} as="span">
...
export function App(props) {
  const router = useRouter();
    ...
    const { setCurrentChecklist } = myContext();
        ...
        () => [
            {
                ...
                disableSortBy: true,
                Cell: ({ row: { original } }) => (
                    <Badge size={"xs"} colorScheme={original.type == "Pediatria" ? "green" : "gray"}>
                        {original.type}
                    </Badge>
                ),
            },
    ...
    return (
        <div className="App" style={{ padding: "20px", width: "100%" }}>
            <AppTable columns={columns} data={props.data} searchEnabled={true} />
        </div>
    );
}
